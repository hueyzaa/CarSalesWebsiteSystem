...
<head>
    ...
    <style>
        ...
        .nav a.active {
            background: linear-gradient(90deg, rgba(255, 215, 0, .18), transparent 70%);
...
<div class="topbar">
    ...
    <div class="quick">
        ...
        <%--        </div>--%>
        <a class="text-warning fw-bold" href="${pageContext.request.contextPath}/logout"><i
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function loadSection(url, activeEl, pushSection) {
            ...
            fetch(url, {headers: {"X-Requested-With": "fetch"}})
                .then(r => {
                    if (!r.ok) throw new Error("Không thể tải nội dung.");
                    return r.text();
                })
                .then(html => {
        ...
        btnCars.addEventListener("click", e => {
            e.preventDefault();
            loadSection(URL_CARS, btnCars, "cars");
        });
        btnPromo.addEventListener("click", e => {
            e.preventDefault();
            loadSection(URL_PROMO, btnPromo, "promotions");
        });
        btnTrans.addEventListener("click", e => {
            e.preventDefault();
            loadSection(URL_TRANS, btnTrans, "transactions");
        });
        btnCust.addEventListener("click", e => {
            e.preventDefault();
            loadSection(URL_CUST, btnCust, "customers");
        });
        ...
        switch (section) {
            case "promotions":
                loadSection(URL_PROMO, btnPromo, null);
                break;
            case "transactions":
                loadSection(URL_TRANS, btnTrans, null);
                break;
            case "customers":
                loadSection(URL_CUST, btnCust, null);
                break;
            case "cars":
            default:
                loadSection(URL_CARS, btnCars, null);
                break;
        }
